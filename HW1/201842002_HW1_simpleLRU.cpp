...
LRUCache::LRUCache(int c, int a, int s) {
	capacity = c;
	associativity = a;
	block_size = s;
...
void LRUCache::Access(const char* cmd, std::string address) {
	...
	if (target_loc == node_h[target_index].end()) {
		...
		if (node_h[target_index].size() == associativity) {
			// Cache index is full.
			// Then remove LRU and add value to MRU.
			target_loc = std::prev(target_loc, 1);
			cache_eviction[target_loc->dirty] += 1;
			node_h[target_index].pop_back();
		} else {
			// Cache index is not full.
			// Then add value to MRU.
		}
	} else {
		// In cache
		// Send to MRU
		node_h[target_index].erase(target_loc);
		cmd_i |= target_loc->dirty;
	}
	node_h[target_index].push_front(Node{target_tag, cmd_i});
}

void LRUCache::Statistics(std::string path_out) {
...
int main(void) {
	...
	LRUCache cache = LRUCache(config[0],config[1],config[2]);
